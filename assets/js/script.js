            ...
            fetch(fiveDayUrl)
                ...
                .then (function(forecastData){
                    ...
                    for (var i = 0; i < 5; i++){
                        ...
                        cardDate.textContent = dayjs().add(i,"days").format("DD/MM/YYYY");
                        forecastCard.append(cardDate);

                        var cardIcon = document.createElement("img");
                        var iconCode = forecastData.daily[i].weather[0].icon;
                        cardIcon.setAttribute("src", "http://openweathermap.org/img/wn/" + iconCode + ".png");
                        forecastCard.append(cardIcon);

                        var cardTemp = document.createElement("p");
                        cardTemp.textContent = "Temperature: " + forecastData.daily[i].temp.day + " F";
                        forecastCard.append(cardTemp);

                        var cardWind = document.createElement("p");
                        cardWind.textContent = "Wind Speed: " + forecastData.daily[i].wind_speed + " MPH";
